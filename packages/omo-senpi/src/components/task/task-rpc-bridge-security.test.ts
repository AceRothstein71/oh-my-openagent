...
describe("event-bridge task RPC security and bounds", () => {
  ...
  it("#given a foreign task name and scope-denied send #when controls run #then cancel fails closed and send redacts owner guidance", async () => {
  ...
  it("#given oversized controls and snapshots #when RPC handles them #then inputs reject and output truncates explicitly", async () => {
