...
function registerTaskHandlers(
  ...
  handle("omo.task.send", async (data) => {
    ...
    const details = (await runTaskSend(engine.manager, input.value, sessionId)).details
    return details.kind === "scope_denied" ? scopeDenied(details.task_id) : details
  })
  handle("omo.task.cancel", async (data) => {
    ...
    if (record === undefined) return { kind: "not_found", reason: "Task not found." }
    if (record.parent_session_id !== sessionId) return scopeDenied(record.task_id)
...
function scopeDenied(taskId: string) {
  return {
    kind: "scope_denied",
    task_id: taskId,
    reason: "Task is outside the current parent session.",
  } as const
}
