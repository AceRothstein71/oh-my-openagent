...
- Full OmO Senpi gate:
  - `bun run test:senpi`
  - observed: generated artifacts rebuilt, package typecheck passed, 1,140 tests passed across 168 files, 0 failed.
- Full task engine:
  - `bun test packages/senpi-task`
  - observed: 1,363 tests passed across 203 files, 0 failed.
...
- Generated artifact integrity:
  ...
  - main `omo.js`: 604,336 bytes.
  - lazy `omo-task.js`: 482,566 bytes.
- Patch hygiene:
  - `git diff --check`
  - observed: exit code 0.
...
- Scope-denied responses reveal neither the owner session id nor `all_scope` escalation guidance.
